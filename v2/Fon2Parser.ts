...
import { Color, Fon2Info, Fon2Palette, Fon2Img } from './fon2';
import { createCanvas, createImageData } from 'canvas';

export class Fon2Parser {

  public static extractPalette(data: DataView, paletteSize: number, offset: number): Fon2Palette {
    const palette: Color[] = [];
    let minLightness = 255;
    let maxLightness = 0;

    for (let i = 0; i < paletteSize + 1; i++) {
      const color: Color = [data.getUint8(offset), data.getUint8(offset + 1), data.getUint8(offset + 2)];
      offset += 3;
      palette.push(color);
  ...
  public static extractCharacterWidths(dv: DataView, { constantWidth, firstCharIndex, lastCharIndex, hasKerningInfo }: Fon2Info): { widths: number[], offset: number } {

    let offset = 12 // byte padding before the first character width

    const kerningInfo = hasKerningInfo ? Boolean(dv.getInt16(offset, true)) : false
  ...
  public static extractGlyphs(dv: DataView, { characterHeight, firstCharIndex, lastCharIndex, paletteSize }: Fon2Info, { palette }: Fon2Palette, charcterWidths: number[], offset: number) {
